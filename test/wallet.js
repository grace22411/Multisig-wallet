...
contract("Wallet", (accounts) => {
  ...
  it("should have correct approvers and quorum", async () => {
    ...
    assert(quorum.toNumber() === 2); //if its a big numner, use toString()
  });
  ...
  it("should NOT create transfer if sender is not approved", async () => {
    await expectRevert(
      wallet.createTransfer(100, accounts[5], { from: accounts[7] }),
  ...
  it('should NOT aprrove transfer if sender is not approved', async () => {
    await wallet.createTransfer(100, accounts[5], { from: accounts[0] });
    await expectRevert(
  ...
  it('should NOT aprrove transfer if transfer is already sent', async () => {
    await wallet.createTransfer(100, accounts[6], { from: accounts[0] });
    await wallet.approveTransfer(0, {from:accounts[0]});
    await wallet.approveTransfer(0, {from:accounts[1]});
    await expectRevert(
  ...
  it('should NOT aprrove transfer twice', async () => {
    await wallet.createTransfer(100, accounts[6], { from: accounts[0] });
    await wallet.approveTransfer(0, {from: accounts[1]});
    await expectRevert(
